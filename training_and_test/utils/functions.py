...
def check_labels_distribution(mega_dict, coordinates):
    """
    Computes the average label value for a given set of coordinates.

    Parameters:
    mega_dict (dict): A dictionary containing contig information, where each contig has a 'labels' key 
                      that maps to a list of label values.
    coordinates (list of tuples): A list of (contig, position) tuples, where 'contig' is a key in mega_dict,
                                   and the position refers to the index within that contig to sample data from.

    Returns:
    float: The average label value across all provided coordinates.
    """
    labels = 0
...
class DataGenerator(Dataset):
    """
    Data generator for training a model, generating data based on a set of coordinates and a window size.
...
def create_data_loaders(label, window_size, batch_size):
    """
    Loads the dataset and creates data generators and DataLoader objects for training and validation.

    This function imports the dataset from JSON files, processes the data into the required format, 
    and initializes DataGenerator instances for training and validation. It then wraps these generators 
    in PyTorch DataLoader objects for efficient batch processing.
    ...
    **kwargs: create_coordinates_one_set parameters
    ...
    for i, path in enumerate(paths):
        ...
        for contig_name in chim:
            ...
            labels = make_labels(coverages, labels_type) 
            if labels_type == 'EuristicLabels':
                chim[contig_name]['labels'] = merge_labels(labels)
            elif labels_type == 'BinaryPositiveMean':
                chim[contig_name]['labels'] = labels
        print('Labels created.')  
