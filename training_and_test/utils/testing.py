...
def metamodel_import(model_path, modA, modB, summary=True):
    model_name = (model_path.split('/')[12]).split('.')[0]
    init_model = getattr(models, model_name)
    model = init_model(modA, modB)
    weights = torch.load(model_path, map_location=torch.device('cpu'))
    model.load_state_dict(weights)
    if summary:
        model.summary()
...
### CONFUSION MATRICES ###
def confusion_matrix_figures(model, label_type, test_level, info_path):
    # paths
    data_path = '/shares/CIBIO-Storage/CM/scratch/users/marco.chiloiro/thesis/models/final_version/training/data/'
    coordinates_path = data_path +f'test_{test_level}_coordinates.txt'
    if test_level == 1:
    ...
    with torch.no_grad():
        for inputs, labels in progress_bar:
            ...
            cm += confusion_matrix(labels.cpu().numpy(), outputs.cpu().numpy(), labels=[0,1])
    ...
    plt.tight_layout()
    save_path = os.path.dirname(info_path)
    plt.savefig(save_path+f'/cm_test_{test_level}.png', format='png')
    print(f"Plot saved as {save_path+f'/cm_test_{test_level}.png'}")


def combined_confusion_matrices(models, label_type, test_level, info_path, file_name):
    data_path = '/shares/CIBIO-Storage/CM/scratch/users/marco.chiloiro/thesis/models/final_version/training/data/'
    coordinates_path = data_path +f'test_{test_level}_coordinates.txt'
    if test_level == 1:
        mega_dict_name = 'train_mega_dict.json'
    ...
    with torch.no_grad():
        for inputs, labels in progress_bar:
            ...
            for model in models:
                outputs = model(inputs)
                accumulated_outputs += outputs 
            outputs = (accumulated_outputs/len(models) >= 0.5).int()
            labels = (labels >= 0.5).int()
            cm += confusion_matrix(labels.cpu().numpy(), outputs.cpu().numpy(), labels=[0,1])
    cm = cm.astype('float')/cm.sum()
    # Plot
    ...
    plt.ylabel('True label', fontsize=13)
    plt.tight_layout()
    save_path = f'/shares/CIBIO-Storage/CM/scratch/users/marco.chiloiro/thesis/models/final_version/results/{label_type}/{file_name}_cm_test_{test_level}.png'
